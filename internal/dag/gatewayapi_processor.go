...
func (p *GatewayAPIProcessor) computeHTTPRoute(route *gatewayapi_v1beta1.HTTPRoute, routeAccessor *status.RouteParentStatusUpdate, listener *listenerInfo) (bool, sets.String) {
	...
	for ruleIndex, rule := range route.Spec.Rules {
		...
		for _, filter := range rule.Filters {
			...
			case gatewayapi_v1alpha2.HTTPRouteFilterURLRewrite:
				// Get the redirect filter if there is one. Note that per Gateway API
				// docs, "specifying a core filter multiple times has unspecified or
				// custom conformance.", here we choose to just select the first one.
				if filter.URLRewrite != nil {
					switch filter.URLRewrite.Path.Type {
					case gatewayapi_v1alpha2.PrefixMatchHTTPPathModifier:
						if (len(filter.URLRewrite.Path.Substitution) > 0) {
							prefixRewrite = filter.URLRewrite.Path.Substitution
						} else {
							prefixRewrite = "/"
						}
					default:
		...
		if redirect != nil {
			...
			routes = p.clusterRoutes(route.Namespace, matchconditions, headerPolicy, mirrorPolicy, rule.BackendRefs, routeAccessor, priority, prefixRewrite)
		}
...
func (p *GatewayAPIProcessor) clusterRoutes(routeNamespace string, matchConditions []*matchConditions, headerPolicy *HeadersPolicy, mirrorPolicy *MirrorPolicy, backendRefs []gatewayapi_v1beta1.HTTPBackendRef, routeAccessor *status.RouteParentStatusUpdate, priority uint8, prefixRewrite string) []*Route {
